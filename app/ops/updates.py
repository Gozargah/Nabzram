...
def get_xray_version_info() -> Dict[str, Any]:
    ...
    try:
        ...
        return error_reply(f"Failed to get version info: {str(e)}")
...
def update_xray(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if request_version:
            ...
            if (
                request_version not in available
                and f"v{request_version}" not in available
            ):
        ...
        if ok:
            ...
            try:
                if (
                    ...
                    and process_manager.is_server_running(
                        process_manager.current_server_id
                    )
                ):
...
def update_geodata() -> Dict[str, Any]:
    ...
    try:
        if process_manager.is_any_server_running():
            ...
            if info:
                process_manager.start_single_server(
                    info.server_id, info.subscription_id, info.config
                )
